...
async def download_osu_file(beatmap_id):
    try:
        r, data = await fetch(f"https://osu.ppy.sh/osu/{beatmap_id}")
        if r is None or data is None:
            raise ValueError("Failed to fetch beatmap")
        if not data.startswith(b"osu file format"):
            raise ValueError("Invalid osu file")
        tf = tempfile.NamedTemporaryFile(delete=False, suffix='.osu')
        tf.write(data)
        tf.close()
        return tf.name
    except Exception as e:
        log.error(f"Download failed for {beatmap_id}: {e}")
        return None
...
async def cmd_dw(_, message):
    ...
    msg = await message.reply_text(t(uid, "downloading"))
    path = f"{beatmapset_id}.osz"
    try:
        await download_with_progress(f"https://osu.ppy.sh/beatmapsets/{beatmapset_id}/download?noVideo=1", path)
        await message.reply_document(path)
        await msg.delete()
    except Exception as e:
        await msg.edit_text(t(uid, "download_error", e=str(e)))
    finally:
