...
DROID_TOKEN = os.getenv("DROID_TOKEN")
API_BASE = os.getenv("API_BASE")

# cached oauth token
OSU_TOKEN_CACHE = {"token": None, "expiry": 0}
...
async def cmd_topplays(_, message):
    ...
    url = f"https://new.osudroid.moe/api2/frontend/profile-username/{quote_plus(username)}"
    ...
    try:
        ...
        try:
            ...
            log.error("Invalid JSON from API: %s", data)
            return await message.reply_text(t(uid, "api_error"))

        # Проверяем топ-игры
        plays = profile.get("Top50Plays") or []
        if not plays:
            return await message.reply_text(t(uid, "no_topplays"))

        # Сортируем топ-игры по PP
        sorted_plays = sorted(plays, key=lambda x: x.get("MapPP", 0), reverse=True)
        ...
        # Берем первую топ-игру
        play = sorted_plays[0]
        mods = "".join(m.get("acronym", "") for m in (play.get("Mods") or [])) or "NM"
        acc = round(play.get("MapAccuracy", 0) * 100, 2)
        pp = play.get("MapPP", 0)
        filename = play.get("Filename", "Unknown")

        text = (
            f"Player: {profile.get('Username', username)}\n"
            f"Top play 1/{len(plays)}\n"
            f"{filename}\n"
            f"PP: {pp:.2f} | Acc: {acc}% | {mods}"
        )

        await message.reply_text(text, reply_markup=topplays_keyboard(0, len(plays)))

    except Exception:
        log.exception("Unexpected error in Topplays command")
        await message.reply_text(t(uid, "api_error"))
